...
async function getRequestCount(session: string, token: string) {
  'use cache';
...
async function getRequestsThisWeek(session: string, token: string) {
  'use cache';
  const authed = client.withAuth(session, token);
  const { data, error } = await authed.reservations().getRequestsThisWeek({});
  cacheTag('requests');
  return { data, error };
}
...
export default async function DashboardPage() {
  ...
  const { data: totalCount, error: countError } = await getRequestCount(
    session,
    token,
  );
  const { data: thisWeekData, error: weeklyCountError } =
    await getRequestsThisWeek(session, token);
  ...
  return (
    <div className='flex flex-col'>
      <div className='flex-1 space-y-4 p-8'>
        ...
        <div className='flex md:grid md:grid-cols-4 justify-between'>
          ...
          <Card className='col-span-4'>
            ...
            <CardContent className='flex'>
              <Suspense fallback={<Skeleton className='h-[300px] w-[500px]' />}>
                <ByMonthLine dataPromise={AggregateChartData(session, token)} />
              </Suspense>
            </CardContent>
          </Card>
        </div>
